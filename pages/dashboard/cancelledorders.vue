                    ...
                    <div class="shadow container-fluid data-bg p-3">
                        ...
                        <div class="table-responsive h-100">
                            <table class="table align-middle table-hover text-center regular">
                                ...
                                <tbody>
                                    <tr :key="order.id" :id='"goTo"+order.id' v-for="order in cancelled" @click="goToDetails(order.id)">
                                        <td>{{ order.id }}</td>
...
<script>
import { cancelledOrdersAsyncData } from '../../util/asyncData/dashboard/cancelledorders'
export default {
    async asyncData({$fire}) {
         return await cancelledOrdersAsyncData($fire);
    },
    methods: {
        goToDetails(orderId){
